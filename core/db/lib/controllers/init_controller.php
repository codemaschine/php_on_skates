...
class MpmInitController extends MpmController {
  ...
  public function doAction() {
    ...
    */
    ...
    if (!$db_config->migrations_table) {
      $db_config->migrations_table = 'db_migrations';
    }
    $migrations_table = $db_config->migrations_table;

    echo 'Looking for existing migrations table... ';
    ...
    try {
      if (false === MpmDbHelper::checkForDbTable()) {
        ...
        $sql2 = "ALTER TABLE `{$migrations_table}` ADD UNIQUE INDEX `TIMESTAMP_INDEX` ( `timestamp` )";
  ...
  public function displayHelp() {
    ...
    $obj->addText('This command is used to initialize the migration system for use with your particular deployment. In particular, it builds the migration table in your database. You need to enter your database information in ../commons/config.php first.');
    //$obj->addText(' ');
    //$obj->addText('Example:');
