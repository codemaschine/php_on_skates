...
class MpmMigrationHelper {
   ...
   * @return array
   */
  static public function getListOfMigrations(&$toId = NULL, $direction = 'up', $only_one_step = false) {
    $db_config = $GLOBALS['db_config'];
    $migrations_table = $db_config->migrations_table;
    $list = [];
    ...
    if ($direction == 'up') {
      $sql = "SELECT `id`, `timestamp` FROM `{$migrations_table}` WHERE `active` = 0".($timestamp ? " AND `timestamp` <= '$timestamp'" : '').' ORDER BY `timestamp`'.($only_one_step ? ' LIMIT 1' : '');
    } else {
      $sql = "SELECT `id`, `timestamp` FROM `{$migrations_table}` WHERE `active` = 1".($timestamp ? " AND `timestamp` > '$timestamp'" : '').' ORDER BY `timestamp` DESC'.($only_one_step ? ' LIMIT 2' : '');
    }
    switch(MpmDbHelper::getMethod()) {
      case MPM_METHOD_PDO:
        try {
