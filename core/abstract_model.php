...
abstract class AbstractModel implements JsonSerializable {
  ...
  protected static $default_find_options = [];
  protected static $default_to_array_except_option = [];
  ...
  protected $mass_assignable = []; // mass assignable fields. If empty, the protection is switched of
  protected $public_fields = []; // fields that can safely returned to public on an API-call (JSON-API)
  protected $private_fields = []; // fields that can additionally returned to the records owner on an API-call (JSON-API)
  protected $attr_defs;

  public $attr = [];
  private $attr_loaded = []; // peristed (clean) attributes.
  public function get_clean_attr($attr = null) {
    if ($attr === null) {
      return $this->attr_loaded;
    }
    return $this->attr_loaded[$attr] ?? null;
  }
  public function get_persisted_attr() {
    return $this->get_clean_attr();
  }  // alias for get_clean_attr()
  ...
  protected function before_attr_loaded() {
  }
  protected function after_attr_loaded() {
  }
   ...
   */
  public function __construct($attr = [], $secure_merge = true) {
  ...
  protected function has_many($name, $options = []) {
  ...
  protected function belongs_to($name, $options = []) {
    if (empty($options['class_name'])) {
  ...
  protected function has_one($name, $options = []) {
    if (empty($options['class_name'])) {
      $options['class_name'] = strtouppercamelcase($name);
    }
  ...
  public function set($name, $value) {
    ...
    } elseif (!empty($this->attr_defs[$name]) && $this->attr_defs[$name] == 'attachment' && $value == NULL) {
      $this->attr[$name] = NULL;
  ...
  private function _save($skip_validation = false, $update_fields = NULL) {
    ...
    if (empty($this->attr)) {
      throw new Exception("Define some fields in '".$this->get_table_name()."'!", E_USER_ERROR);
    }

    if (!empty($this->attr_defs['created_at']) && $this->is_new()) {
      $this->attr['created_at'] = $this->attr_defs['created_at'] === 'datetime' ? new DateTime() : time();
    }

    if (!empty($this->attr_defs['updated_at'])) {
    ...
    foreach ($this->attr as $key => $value) {
      ...
      if ($this->is_new() || $this->attr_loaded[$key] !== $value) { // only update if the the attribute is really dirty!!!
        ...
        } // avoid the usage of , instead of . as decimal separator caused by setlocale.
        elseif ($value instanceof DateTime) { // works for Date-class, too!
          $value = "'".$value->toDbFormat()."'";
        } elseif (!is_numeric($value)) {
          $value = 'NULL';
        } // Default: drop other values
  ...
  public function destroy() {
    ...
    foreach ($this->relations as &$relation) {
      ...
      switch ($relation_options['dependent'] ?? null) {
        case 'delete': $relation->delete();
          break;
        case 'destroy': $relation->destroy();
          break;
      }
    }
  ...
  public function update_attribute($key, $value, $skip_validation = false) {
    $this->attr[$key] = $value;
    return $this->_save($skip_validation, [$key]);
  }
  ...
  public function is_valid() {
    ...
    foreach ($this->validators as $validator) {
      ...
      if ($validator['name'] != 'presence_of' && !empty($validator['allow_blank']) && empty($attr)) {
        continue;
      }
      ...
      switch ($validator['name']) {
        case 'presence_of':
          ...
          break;
        case 'uniqueness_of':
          $condition_str = !empty($validator['case_sensitive']) ? "binary $attr_name = ?" : "$attr_name = ?";
          if ($this->get_id()) {
            $condition_str .= ' AND id != '.intval($this->get_id());
          }

          if (!empty($validator['scope'])) {
            $fields = is_array($validator['scope']) ? $validator['scope'] : [$validator['scope']];
            $field_cond = [];
          ...
          if (static::find_first(['conditions' => [$condition_str, $attr]])) {
            $this->errors[$attr_name] = $validator['message'];
          }
          break;
        case 'numericality_of':
          if ($attr && (!is_numeric($attr) || !empty($validator['only_integer']) && !is_integer($attr))) {
            $this->errors[$attr_name] = $validator['message'];
          }
          break;
        case 'length_of':
          if (!empty($validator['is']) && mb_strlen($attr) != $validator['is'] ||
              !empty($validator['minimum']) && mb_strlen($attr) < $validator['minimum'] ||
              !empty($validator['maximum']) && mb_strlen($attr) > $validator['maximum']) {
            $this->errors[$attr_name] = $validator['message'];
          }
          break;
        case 'format_of':
          if (!empty($validator['with']) && !preg_match($validator['with'], $attr) ||
              !empty($validator['without']) && preg_match($validator['without'], $attr)) {
            $this->errors[$attr_name] = $validator['message'];
          }
          break;
  ...
  private function add_new_validator($name, $attr_name, $default_message, $options) {
    ...
    $this->validators[] = array_merge($options, ['name' => $name, 'attr_name' => $attr_name]);
  }

  protected function validates_presence_of($attr_name, $options = []) {
    $this->add_new_validator('presence_of', $attr_name, ' darf nicht leer sein.', $options);
  }

  protected function validates_uniqueness_of($attr_name, $options = []) {
    $this->add_new_validator('uniqueness_of', $attr_name, ' ist bereits vergeben.', $options);
  }

  protected function validates_numericality_of($attr_name, $options = []) {
    $this->add_new_validator('numericality_of', $attr_name, !empty($options['only_integer']) ? ' ist keine Ganzzahl.' : ' ist keine Zahl.', $options);
  }

  protected function validates_length_of($attr_name, $options = []) {
    $this->add_new_validator('length_of', $attr_name, !empty($options['is']) ? ' muss genau '.$options['is'].' Zeichen lang sein.' : ' hat eine ungültige Länge.', $options);
  }

  protected function validates_format_of($attr_name, $options = []) {
    $this->add_new_validator('format_of', $attr_name, ' hat ein ungültiges Format.', $options);
  }

  protected function validates_email_of($attr_name, $options = []) {
  ...
  protected function validates_associated($attr_name, $options = ['message' => false]) {
    $this->add_new_validator('associated', $attr_name, '', $options);
  }

  // -- class functions (helpers)

  public static function new_instance($attributes = [], $secure_merge = true) {
    return new static($attributes, $secure_merge);
  }

  public static function create($attributes = [], $secure_merge = true) {
   ...
   */
  private static function find_by($fields = NULL, $options = [], $exception_if_not_found = false) {
    //--- Check if we can use cache
    if (is_array($fields) && is_assoc($fields) && count(array_keys($fields)) == 1 && array_key_exists('id', $fields) &&    // looking only for 'id's
        count(array_diff(array_keys($options), ['take'])) == 0) { // only 'take' is allowed as option
      $temp_ids = is_array($fields['id']) ? $fields['id'] : [$fields['id']];
      $ids = [];
      foreach ($temp_ids as $id) {
        $ids[] = strval($id);
      }     // guarantee that every id is a string
    ...
    if ($options['take'] == self::ALL) {
      if (!empty($options['per_page'])) {  // paginate!!!
  ...
  public static function count_by($fields = NULL, $options = []) {
    global $db_link;
    ...
    $query = 'SELECT '.(!empty($options['group']) ? '1' : 'count(*) as total').' FROM `'.$options['from'].'`'; // if there is 'group by' in the query then count(*) would return counts for each group. So in that case we return 1 and count these result rows later.
    if (!empty($options['joins'])) {
      $query .= ' '.$options['joins'];
    }
  ...
  public static function count_all($options = []) {
  ...
  private static function build_conditions_sql($fields, $options) {
    ...
    if (static::$soft_delete && empty($options['unscoped'])) {
    ...
    if ((static::default_scope()) && empty($options['unscoped']) && empty($options['escape_default_scope'])) {
   ...
   * @return static|static[]|null
   */
  public static function find($ids, $options = [], $exception_if_not_found = true) {
    if (empty($ids)) {
    ...
    return self::find_by(['id' => $ids], $options, $exception_if_not_found);
  }

  /**
   * @return static|null
   */
  public static function find_first_by($fields, $options = [], $exception_if_not_found = false) {
    $options['take'] = self::FIRST;
   ...
   * @return static[]|null
   */
  public static function find_all_by($fields, $options = [], $exception_if_not_found = false) {
    $options['take'] = self::ALL;
    return self::find_by($fields, $options, $exception_if_not_found);
  }

  public static function paginate_all_by($fields, &$total_cnt, $options = [], $exception_if_not_found = false) {
    $options['take'] = self::ALL;
  ...
  public static function find_first($options = [], $exception_if_not_found = false) {
    return self::find_first_by(NULL, $options, $exception_if_not_found);
  }
  public static function find_all($options = [], $exception_if_not_found = false) {
    return self::find_all_by(NULL, $options, $exception_if_not_found);
  }
  public static function paginate_all(&$total_cnt, $options = [], $exception_if_not_found = false) {
  ...
  public function cast_attribute($key) {
    ...
    switch ($default_type) {
      case 'decimal': $default_type = 'float';
        break;
      case 'text':
      case 'binary':
      case 'time': $default_type = 'string';
        break;
      case 'set': $default_type = 'string';
        break;
      case 'primary_key': $default_type = 'integer';
        break;
    }
    ...
    if ($cur_type != $default_type) {
      switch ($default_type) {
        case 'attachment':
          if (!$this->attr[$key] || !is_array($this->attr[$key]) && $this->attr[$key.'_file_name']) {
            $value = new Attachment($key, $this);
          } break;
        case 'int':
        case 'integer': $value = intval($value);
          break; // filter_var($value, FILTER_SANITIZE_NUMBER_INT); break;   // Notice that neither settype() nor filter_var() work here correctly, they can still return a string!!!
        case 'float': $value = floatval($value);
          break;
        case 'date': $value = (new Date($value, Date::getUTCTimeZone()))->setToDefaultTimeZone();
          break;
        case 'datetime': $value = (new DateTime($value, DateTime::getUTCTimeZone()))->setToDefaultTimeZone();
          break;
        case 'bool':
        case 'boolean': $value = is_string($value) ? filter_var($value, FILTER_VALIDATE_BOOLEAN) : $value == true;
          break; // if string, evaluate also string content like "false", "0", "off" and alike. If other type, cast to boolean
        default:
    ...
    if (!empty($options['include']) && (is_string($options['include']) || is_array($options['include']))) {
      ...
      foreach ($options['include'] as $i => $sub) {
        if (!empty($this->relations[$i])) {
          ...
          if (!empty($options['only'][$i])) {
            $sub_options['only'] = $options['only'][$i];
          }
          if (!empty($options['except'][$i])) {
  ...
  protected static function get_from_cache($ids, $limit = 1) {
    ...
    if (empty(self::$cache[get_called_class()])) {
      return NULL;
    }
