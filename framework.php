...
$abs_base_path = dirname(__FILE__);
chdir('app');
...
try {
  if (!$_FRAMEWORK['redirect']) {
    do {
      ...
      require 'commons/pre_controller.php';
      if ($_FRAMEWORK['redirect'] || $_FRAMEWORK['render_type'])  // abort if redirect() or render() is called. Don't go into the controller
        break;
      require 'controller/'.$_FRAMEWORK['controller'];  // execute controller-action
      require 'commons/post_controller.php';
      if ($_FRAMEWORK['redirect'])
        break;
  ...
  if ($_FRAMEWORK['redirect']) {
    require 'commons/post_rendering.php';
    ...
    elseif (is_xhr() || $_FRAMEWORK['render_type'] == 'partial' || !file_exists('views/layout/'.$_FRAMEWORK['layout'])) {
      ...
      include 'views/layout/'.$_FRAMEWORK['layout']; // this also renders the view by calling yieldit();
