...
class MpmDownController extends MpmController
{
	...
	public function doAction()
	{
		...
		*/
		// are we forcing this?
		$forced = false;
		if (in_array('--force', $this->arguments))
		{
		    $forced = true;
		}
		
		$down_to = isset($this->arguments[0]) && is_numeric($this->arguments[0]) ? $this->arguments[0] : NULL;
		$has_down_to_arg = $down_to !== NULL;
		if ($down_to == 0)
		{
		    $down_to = -1;
		}
		if ($has_down_to_arg && !MpmMigrationHelper::getTimestampFromId($down_to)) {
		  echo "Migration '$down_to' not Found! Will not migrate down for security reason! \r\n";
		  return ;
		}
		
		
		// get list of migrations and the current migration number
		$list = MpmMigrationHelper::getListOfMigrations($down_to, 'down', !$has_down_to_arg);
		$total = count($list);
